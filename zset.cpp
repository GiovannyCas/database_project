...
static void tree_insert(ZSet *zset, ZNode *node)
{
    ...
    AVLNode **from = &zset->root;       // the incoming pointer to the nest node
...
// a helper structure for the hashtale lookup
struct HKey 
{
    HNode node;
...
// find the first (name, score) tuple that is >= key
